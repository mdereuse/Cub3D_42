...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/28 12:04:00 by mariedere         #+#    #+#             */
/*   Updated: 2020/02/28 13:43:01 by mariedere        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	write_header(int fd, int size, t_game game)
{
	...
	*((int*)(header + 2)) = size;
	header[10] = 54;
	header[14] = 40;
	*((int*)(header + 18)) = game.screen.width;
	*((int*)(header + 22)) = game.screen.height;
	header[26] = 1;
	header[28] = 24;
...
static int	get_color(int i, int j, t_game game)
{
	...
	color = *((int*)(game.screen.data + 4 * j + game.screen.size_line
											* (game.screen.height - 1 - i)));
	return (color);
}
...
void		screenshot(t_game game)
{
	...
	pad = (4 - ((game.screen.width * 3) % 4)) % 4;
	size = 54 + (3 * game.screen.width + pad) * game.screen.height;
